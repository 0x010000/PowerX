...
func (p *snowFlakeIdWorker) SplitId(length int) int64 {
	IdString := strconv.Itoa(int(p.Id()))
	splitId, _ := strconv.Atoi(IdString[len(IdString)-length:])
	return int64(splitId)
}

func (p *snowFlakeIdWorker) U16() uint64 {
