...
func (e *SceneQrcode) FindByQid(db *gorm.DB, qid string) (qrcode *SceneQrcode) {
	...
	err := db.Table(e.TableName()).Where(`qid = ? AND state < 3`, qid).Find(&qrcode).Error
	if err != nil {
		panic(err)
	}
	return qrcode
...
func (e *SceneQrcode) FindEnableSceneQrcodeByQid(db *gorm.DB, qid string) (qrcode *SceneQrcode) {
	...
	err := db.Table(e.TableName()).Where(`qid = ? AND state = 1`, qid).Find(&qrcode).Error
	if err != nil {
		panic(err)
	}
	return qrcode
...
//  @return qrcode
//
func (e *SceneQrcode) Options(db *gorm.DB, opt *types.SceneQrcodeRequest) (qrcode []*SceneQrcode) {

	query := db.Table(e.TableName()).Where(`state = 1 AND expiry_date > ?`, time.Now().Unix())
	if v := opt.Qid; v != `` {
		query = query.Where(`qid = ?`, v)
	}
	if v := opt.Owner; v != `` {
	...
	if v := opt.ClassifyId; v > 0 {
		query = query.Where(`classify =  ?`, v)
	}
	err := query.Find(&qrcode).Error
	if err != nil {
		panic(err)
	}
	return qrcode
